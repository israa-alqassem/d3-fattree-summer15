...
var fs = require("fs"),
    ...
    .alias("o", "out").describe("o", "out file, defaults to index file + min.src")
...
if (argv.out) {
    out = argv.out;
    if (!hasExtension.test(out)) out += ".src";
} else {
    out = filePath.join(filePath.dir(options.main), filePath.basename(options.main, filePath.ext(options.main))) + ".min.src";
}
